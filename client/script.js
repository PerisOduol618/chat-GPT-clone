...
const handleSubmit = async (e) => {
  ...
  const response = await fetch("http://localhost:5000/", {
    ...
    body: JSON.stringify({
      prompt: data.get("prompt"), //data coming from the text area element on the screen
    }),
  });
  ...
  if (response.ok) {
    const data = await response.json(); // this gives the actual response coming from the backend
    console.log(data)
